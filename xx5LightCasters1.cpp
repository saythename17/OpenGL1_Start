...
int main() {
	...
	myShader.setVec3("light.direction", -.0f, -.0f, -2.0f);
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		myShader.setVec3("light.diffuse", .5f, .5f, .5f);
		myShader.setVec3("light.specular", 1.0f, 1.0f, 1.0f);

		//★material properties
		myShader.setVec3("material.ambient", .2f, .2f, .2f);
		myShader.setVec3("material.diffuse", .5f, .5f, .5f);
		myShader.setVec3("material.specular", .8f, .8f, .8f);
